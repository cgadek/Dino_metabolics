...
Before we dive to deeply into calculated M rates, lets look at the three spectral frequencies and body size using a few multidimensional clustering algorithms to assess and separation in the "raw" lipo-oxidation signatures.
# RAW spectra
```{r}

library(readr)
disc_fossil <- read_csv("data/fossil_raw_spectra.csv")

mds.plot <- disc_fossil%>%
  unite('id', 1:3, remove = T)%>%
  column_to_rownames(., var=c("id"))
...
mds$Taxon_name <- as.factor(disc_fossil$`Taxon Name`)
mds$Thermy <- as.factor(disc_fossil$Thermy)
...
ggscatter(mds, x = "Dim.1", y = "Dim.2",
          ...
          palette = c("grey88", "goldenrod", "dodgerblue3", "red3", "grey22", "grey55", "white"))
...
clust <- kmeans(mds,centers = 4, nstart = 25, )$cluster %>%
  as.factor()
mds <- mds %>%
  mutate(groups = clust)
...
ggscatter(mds, x = "Dim.1", y = "Dim.2", 
          label = disc_fossil$`Taxon Name`,
          fill = "groups",
          palette = c("dodgerblue","red3", "grey", "grey33"),
          size = 3,
          shape=21,
          ellipse = TRUE,
...
dat.mds <- dat.lm%>%
  ...
  dplyr::select(Taxon_Name, termy_3_cat, classification, Log.10..Body.Mass..g.) 


mds.plot <- points%>%
  column_to_rownames(., var="Taxon.Name")%>%
  mutate(log10mass= dat.mds$Log.10..Body.Mass..g.)
...
mds <- mds.plot%>%
  ...
  as_tibble()
colnames(mds) <- c("Dim.1", "Dim.2")
mds$Taxon_Name <- as.factor(rownames(mds.plot))

mds<-mds%>%
  left_join(., dat.mds, by="Taxon_Name")
...
# Plot and color by groups
ggscatter(mds, x = "Dim.1", y = "Dim.2", 
          label = dat.mds$Taxon_Name,
          fill = "groups",
          palette = c( "goldenrod","sienna2"),
          size = 3,
          shape=21,
          ellipse = TRUE,
          font.label = c(8, "plain"),
...
ggscatter(mds, x = "Dim.1", y = "Dim.2",
          ...
          repel = TRUE,
          font.label = c(8, "plain"),
          palette = c("red3","dodgerblue3",  "grey55", "grey88"))
...
ggscatter(mds, x = "Dim.1", y = "Dim.2", 
          label = dat.mds$Taxon_Name,
          fill = "groups",
          palette = c( "goldenrod","sienna2", "bisque3", "grey66"),
          size = 3,
          shape=21,
          ellipse = TRUE,
          font.label = c(8, "plain"),
...
ggplot(data=dat.calib.extant%>%filter(classification %in% c("endo","ecto")), aes(Log.10..Body.Mass..g., log10(Literature.MR..mL.O2...1.h...1.g.), fill=classification, color=classification))+
  geom_smooth(method="lm", alpha=0.1, se=T)+
...
We see that known endotherms in red show a slightly positive slope barely greater than zero—running counter to the expected -1/4 slope derived from previous work. Ecotherms actually do show a nearly -1/4 slope but the association is clearly driven by a single point (Gavialis), making inference dubious.


Let's take a step back and plot the raw amide frequencies against mass.

```{r, echo=F, warning=F, message=F}

ggplot(data=dat.calib.extant%>%filter(classification %in% c("endo","ecto")), aes(Log.10..Body.Mass..g., log10(X.S.N..Amide...MC.in.vivo..normalized.spectral.intensities.), color=classification))+
  geom_point()+
  ...
  scale_color_manual(values=c("blue", "red"))+
  labs(x=expression(paste(Log["10"]~Body~Mass~(g))), y= expression(paste(Log["10"]~"MC"["vivo"])))
...
cooks.distance(lm.calib)
# the Anseriform Duck has a cooks distance of 0.51

#calculate leverage for each observation in the model
hats <- as.data.frame(hatvalues(lm.calib))

#display leverage stats for each observation
hats

#leverge for duck is 0.77
...
contmap_obj <- setMap(obj, colorRampPalette(c("blue", "red"))( 10))
...
# Change the color palette
contmap_obj2 <- setMap(obj2, colorRampPalette(c("black", "white"))( 10))
